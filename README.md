...
### 2.4 文件出题与错题本（本 PR 新增）

| 能力 | 当前实现 | 主要入口 |
|---|---|---|
| 文件出题 | 上传 PDF / Word / PPT / TXT / MD，解析切片后由 AI 生成选择题、填空题、简答题 | `filemate/study/`、Gradio「文件出题」 |
| AI 分析 | 返回知识点数量、内容完整度和推荐题型菜单 | `StudyService.analyze_document` |
| 判题 | 单选前缀匹配、填空互相包含、简答关键词重合率 | `StudyService.submit_answer` |
| 错题本 | 答错自动沉淀，1 / 3 / 7 / 15 / 30 天复习排期，支持复习一次和标记掌握 | Gradio「错题本」 |
| 举一反三 | 按知识点重新生成同类题 | `StudyService.generate_more_questions` |
| 数据模型 | `documents`、`knowledge_chunks`、`file_analyze_results`、`questions`、`answer_records`、`wrong_book_items` | SQLite migration v9 |

AI 不可用或调用失败时会明确报错，不生成无意义的模板题。
